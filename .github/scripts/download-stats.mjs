...
function main(releases, prevState) {
  // No wall-clock field here on purpose: the state (and the report below)
  // change only when the actual counts change, so the workflow commits real
  // updates instead of a daily timestamp-only diff.
  const state = { assets: {} };
...
function renderReport(modules) {
  ...
  lines.push("");
  lines.push(
    "_Auto-generated by [`download-stats`](.github/workflows/download-stats.yml) — " +
      "refreshed daily and on every release. This file changes only when the numbers " +
      "change; see the commit history for when._"
  );
  lines.push("");
  lines.push(
